...
func _ready() -> void:
	...
	_status.hide()
	_status.text = ""
	_status.pixel_size = 0.0009
	_status.font_size = 22
	_status.layers = 2
	_status.no_depth_test = true
	_status.render_priority = 15
	...
	_alarm_light.add_to_group("self_lit")
	...
	for node in get_parent().find_children("*", "Light3D", true, false):
		if node != _alarm_light and not node.is_in_group("self_lit"):
			_lights.append(node)
	for node in get_parent().get_children():
		if node is Light3D and not node in _lights and not node.is_in_group("self_lit"): _lights.append(node)
...
func _process(delta: float) -> void:
	...
	_pulse += delta
	_fever_blend = move_toward(_fever_blend,1.0 if GameManager.fever_left > 0 else 0.0,delta*1.5)
	_stun_blend = move_toward(_stun_blend,1.0 if GameManager.stun_left > 0 else 0.0,delta*2)
	_screen_mat.set_shader_parameter("fever", _fever_blend)
	_screen_mat.set_shader_parameter("stun", _stun_blend)
	_hostile_timer -= world_delta
	if _hostile_timer <= 0 and not GameManager.is_run_over() and not GameManager.tutorial_active:
		_hostile_timer = maxf(3.5, 16.0 - GameManager.depth * 2.2)
		if GameManager.play_mode == GameManager.PlayMode.EDUCATIONAL: _hostile_timer = 18.0
	...
	_status.visible = alive
	var fever := GameManager.fever_left > 0 and alive
	_screen.visible = _fever_blend > .001 or _stun_blend > .001
	...
	if GameManager.play_mode == GameManager.PlayMode.EDUCATIONAL and _stun_blend < .01:
		illumination = maxf(.10,illumination)
	if _env:
		...
		if GameManager.play_mode == GameManager.PlayMode.EDUCATIONAL:
			var zone := minf(4.999,(300.0-GameManager.expedition_left)/60.0)
			var colors: Array[Color] = [Color("267f87"),Color("2c6252"),Color("1c547d"),Color("263454"),Color("11192f")]
			var previous := maxi(0,int(zone)-1)
			_env.fog_light_color = colors[previous].lerp(colors[int(zone)],smoothstep(0,.12,fposmod(zone,1)))
	...
	var alarm_on := alive and health <= 0.5
	...
	if alive:
		...
		if GameManager.play_mode == GameManager.PlayMode.EDUCATIONAL:
			_mission.text += "\n"+GameManager.ECOSYSTEMS[mini(4,int((300-GameManager.expedition_left)/60))]
		_status.text = ""
...
func _contamination() -> void:
	...
	for fish in get_tree().get_nodes_in_group("fish"):
		if fish.unsuitable or fish._clicked or fish.collision_layer == 0 or fish._age < 6.0: continue
...
func _depth_announcement(value: int) -> void:
	...
	_descend.show()
	_descend.collision_layer = 0
	_descend.get_node("Label3D").text = "↓ %d km" % value
...
func _defeat(value: bool) -> void:
	...
	for entity in get_tree().get_nodes_in_group("entities"):
		...
		fade.tween_property(entity, "scale", Vector3.ONE * 0.01, 1.8).set_trans(Tween.TRANS_SINE)
		fade.tween_callback(entity.queue_free)
	for cleaner in get_tree().get_nodes_in_group("cleaners"): cleaner.set_physics_process(false)
	GameManager.active_cleaners = 0
	for hostile in get_tree().get_nodes_in_group("hostiles"):
		hostile.collision_layer = 0
		var fade := create_tween()
		fade.tween_property(hostile, "scale", Vector3.ONE * 0.01, 1.5)
		fade.tween_callback(hostile.queue_free)
	_status.hide()
	...
	_restart.show()
	_restart.scale = Vector3.ONE * 0.65
	create_tween().tween_property(_restart, "scale", Vector3.ONE, 0.8).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
...
func _bubbles(at: Vector3) -> void:
	if not PlayerJournal.particles: return
...
func _update_descent(delta: float) -> void:
	...
	if _descent_left > 0:
		_descend.get_node("Label3D").text = "↓ %d km" % GameManager.depth
